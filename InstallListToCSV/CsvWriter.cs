...
public class CsvWriter : IDisposable
{
    ...
    public CsvWriter(string path, Encoding encoding)
    {
        ...
        {
            Console.WriteLine("{0}が別のプロセスで使用されています。\n{1}として保存を開始します。", fileName, fileNameEx);
            stream = new FileStream(fileNameEx, FileMode.Create, FileAccess.ReadWrite);
        }
